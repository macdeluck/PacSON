		...
		IOnSceneTouchListener, IPinchZoomDetectorListener
{
	...
	public EngineOptions onCreateEngineOptions()
	{
		...
		/*
		 * int factor = CAMERA_HEIGHT * CAMERA_WIDTH; if (factor < 120000) {
		 * CAMERA_WIDTH *= 2; CAMERA_HEIGHT *= 2; } else if (factor < 400000) {
		 * CAMERA_WIDTH = (int) (CAMERA_WIDTH * 1.5) / BLOCK_WIDTH BLOCK_WIDTH;
		...
		// (AREA_HEIGHT - CAMERA_HEIGHT) / 2, CAMERA_WIDTH, CAMERA_HEIGHT);//
		// cam;
	...
	public boolean onSceneTouchEvent(Scene pScene, TouchEvent pSceneTouchEvent)
	{
		...
		if (pSceneTouchEvent.isActionMove())
		{
			...
			camera.setCenter(camera.getCenterX() + touchOffsetX,
					camera.getCenterY() + touchOffsetY);
		}
		return true;
	}
	...
	// well same behavior like in normal zoom plus save zoom factor
	@Override
